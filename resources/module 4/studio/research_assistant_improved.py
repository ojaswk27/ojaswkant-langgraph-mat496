...
def search_duckduckgo(state: InterviewState):
    ...
    try:
        ...
        formatted_search_docs = f"DuckDuckGo search failed: {str(e)}"

    return {"context": [formatted_search_docs]} 

def search_semantic_scholar(state: InterviewState):
    
    """ Retrieve docs from Semantic Scholar """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            formatted_search_docs = f"Semantic Scholar search returned status {response.status_code}"
    except Exception as e:
        formatted_search_docs = f"Semantic Scholar search failed: {str(e)}"
...
def write_section(state: InterviewState):
    ...
    system_message = section_writer_instructions.format(focus=analyst.description, context=context)
    section = llm.invoke([SystemMessage(content=system_message)]+[HumanMessage(content=f"Use this interview transcript to write your section: {interview}")]) 
...
def write_report(state: ResearchGraphState):
    ...
    system_message = report_writer_instructions.format(topic=topic, context=formatted_str_sections, fact_check_results=fact_check_results)    
...
builder.add_edge(["write_conclusion", "write_report", "write_introduction", "write_executive_summary"], "finalize_report")
